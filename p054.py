...
player2 = []
for x in rounds:
    player1.append("".join(x[0:5]))
    player2.append("".join(x[5:10]))


card_value = {'2' : 1 , '3': 2 , '4':3, '5':4, '6':5, '7':6, '8':7, '9':8, 'T':9, 'J':10, 'Q': 11, 'K':12, 'A': 13,  'C' : 0, 'D' : 0, 'S': 0, 'H':0}
inverse = {v:k for k,v in card_value.items()}
hand = 'TCJCQCKCAD'
def hand_value(hand):
    if all(inverse[i] in hand for i in range(9, 14)) and all(hand[1] == hand[2*k +1] for k in range(5)):
        return 10
    for i in range(1, 9):
        if all(inverse[i+c] in hand for c in range(5)) and all(hand[1] == hand[2*k +1] for k in range(5)):
            return(9)
    for i in range(1, 14):
        if hand.count(inverse[i]) == 4:
            return(8)
    for i in range(1, 14):
            if hand.count(inverse[i]) == 3:
                for j in range(1, 14):
                    if j!=i and hand.count(inverse[j]) == 2:
                        return(7)
    if all(hand[1] == hand[2*k +1] for k in range(5)):
        return(6)
    for i in range(1, 10):
            if all(inverse[i+c] in hand for c in range(5)):
                return(5)
    for i in range(1, 14):
        if hand.count(inverse[i]) == 3:
            return(4)
    for i in range(1, 14):
        if hand.count(inverse[i]) == 2:
            for j in range(1, 14):
                if j!=i and hand.count(inverse[j]) == 2:
                    return(3)
    for i in range(1, 14):
...
for i in range(len(player1)):
    value1 = hand_value(player1[i])
    value2 = hand_value(player2[i])
    if value1 > value2:
        p1wins+=1
    if value1 == value2 == 9:
        if max(card_value[x] for x in player1[i]) > max(card_value[x] for x in player2[i]):
            p1wins+=1
    if value1 == value2 == 8:
        for j in range(1,14):
            if player1[i].count(inverse[j]) == 4:
                quad1 = j
        for j in range(1,14):
            if player2[i].count(inverse[j]) == 4:
                quad2 = j
        if quad1 > quad2:
             p1wins+=1
        if quad1==quad2:
            for j in range(1,14):
                if player1[i].count(inverse[j]) == 1:
                    extra1 = j
            for j in range(1,14):
                if player2[i].count(inverse[j]) == 1:
                    extra2 = j
            if extra1 > extra2:
                p1wins+=1
    if value1 == value2 == 7:
        for j in range(1,14):
            if player1[i].count(inverse[j]) == 3:
                trips1 = j
        for j in range(1,14):
            if player2[i].count(inverse[j]) == 3:
                trips2 = j
        if trips1 > trips2:
            p1wins+=1
        if trips1 == trips2:
            for j in range(1,14):
                if player1[i].count(inverse[j]) == 2:
                    extra1 = j
            for j in range(1,14):
                if player2[i].count(inverse[j]) == 2:
                    extra2 = j
            if extra1 > extra2:
                p1wins+=1
    if value1 == value2 == 6:
    ...
    if value1 == value2 == 4:
        ...
        if trips1 > trips2:
            p1wins+=1
        if trips1 == trips2:
            if max(card_value[x] for x in player1[i] if x!=inverse[trips1]) > max(card_value[x] for x in player2[i] if x!=inverse[trips1]):
                    p1wins+=1
    if value1 == value2 == 3:
        if max(card_value[x] for x in player1[i] if player1[i].count(x) == 2) > max(card_value[x] for x in player2[i] if player2[i].count(x) == 2):
            p1wins+=1
        if max(card_value[x] for x in player1[i] if player1[i].count(x) == 2) == max(card_value[x] for x in player2[i] if player2[i].count(x) == 2):
            if min(card_value[x] for x in player1[i] if player1[i].count(x) == 2 and card_value[x]>0) > min(card_value[x] for x in player2[i] if player2[i].count(x) == 2 and card_value[x]>0):
                 p1wins+=1
            if min(card_value[x] for x in player1[i] if player1[i].count(x) == 2) > min(card_value[x] for x in player2[i] if player2[i].count(x) == 2):
                for j in range(1,14):
                    if player1[i].count(inverse[j]) == 1:
                        extra1 = j
                for j in range(1,14):
    ...
    if value1 == value2 == 2:
        ...
        if pair1 > pair2:
             p1wins+=1
        if pair1 == pair2:
            extra1 = max(card_value[x] for x in player1[i] if x != inverse[pair1])
            extra2 = max(card_value[x] for x in player2[i] if x != inverse[pair2])
            if extra1 > extra2:
                 p1wins+=1
    if value1 == value2 == 1:
        extra1 = max(card_value[x] for x in player1[i])
        extra2 = max(card_value[x] for x in player2[i])
        if extra1 > extra2:
            p1wins+=1
        if extra1 == extra2:
            add1 = max(card_value[x] for x in player1[i] if x!= inverse[extra1])
            add2 = max(card_value[x] for x in player2[i] if x!= inverse[extra2])
            if add1 > add2:
                 p1wins+=1
print(p1wins)
